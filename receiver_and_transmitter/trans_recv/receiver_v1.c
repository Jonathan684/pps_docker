...
int main(){
    ...
    ret_input = iio_channel_attr_write(chnn_device_input, "sampling_frequency", "700000");//3999999
    if (ret_input < 0) {
    ...
    */
    ret_input = iio_channel_attr_write(chnn_device_input, "gain_control_mode", "slow_attack");//# Receive path AGC Options: slow_attack, fast_attack, manual
    if (ret_input < 0) {
    ...
    for (p_dat = (char *)iio_buffer_first(rxbuf, rx0_i); p_dat < p_end; p_dat += p_inc) {
        ...
        //printf("%d %f %d\n",increm,rxSignal[increm].im,flag);

...
size_t TxBufferSize     =  (pow(2, 15)-1);//150000;// (pow(2, 17)-1);//(pow(2, 14)-1);
size_t RxBufferSize     = (pow(2, 18)-1);//(pow(2, 18)-1);

/* PULS0*/
int Longitud_del_pulso  = 1;
int PRI  = (pow(2, 15)-1);  //150000;//(pow(2, 17)-1);   // TxBufferSize   //16383;//4; //Minimo 4 
int16_t amplitud = 0x0FFF ; //4080;  // 0x0FFF;  // Jupyter
int N_rx = (pow(2, 18)-1);  //RxBufferSize
bool rx  = true;
...
int main(){
    ...
    if(rx == true){
            ...
            for (p_dat = (char *)iio_buffer_first(rxbuf, rx0_i); p_dat < p_end; p_dat += p_inc) {     
                if(flag > PRI){//se tomaran muestras despues de leer un buffer de tamaño del transmisor
                if(I_rx < N_rx){

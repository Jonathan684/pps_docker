...
int PRI = 4; //Minimo 4 
int16_t amplitud = 0x0FFF;// jupyter
int N_rx = (pow(2, 18)-1);
bool rx = true;
...
int main(){
    ...
    if(rx == true){
            ...
            for(int j=0;j<N_rx;j++){
                    if(signal_i[j]>=0)sprintf(output, " %f +%fj,",(signal_q[j]/amplitud),(signal_i[j]/amplitud));   
                    else sprintf(output, " %f %fj,", (signal_q[j]/amplitud), (signal_i[j]/amplitud));

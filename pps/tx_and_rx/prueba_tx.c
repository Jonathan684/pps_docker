...
int main(){
    ...
    int ret = iio_channel_attr_write(chnn_device_input, "sampling_frequency", "60000000");//3999999
    if (ret < 0) {
       perror("Error setting sampling_frequency rate RX: "); 
        return ret;
    }
    //else printf("Set correct sampling_frequency RX \n");
    ret_input = iio_channel_attr_write(chnn_device_input, "rf_bandwidth", "20000000");//3999999
    if (ret_input < 0) {
    ...
    ret_output = iio_channel_attr_write(chnn_device_output, "sampling_frequency", "60000000");
    if (ret_output < 0) {
       perror("Error setting sampling_frequency rate TX : "); 
        return ret_output;
    }
    //else printf("Set correct sampling_frequency TX  \n");
    ret_output = iio_channel_attr_write(chnn_device_output, "rf_bandwidth", "20000000");
    if (ret_output < 0) {
       perror("Error setting rf_bandwidth rate TX: "); 
        return ret_output;
    }
   
    ret_output = iio_channel_attr_write(chnn_device_input, "gain_control_mode", "fast_attack");//# Receive path AGC Options: slow_attack, fast_attack, manual
    if (ret_output < 0) {
    ...
    ret_output = iio_channel_attr_write(chnn_altvoltage1_output, "frequency", "3500000000");//# Receive path AGC Options: slow_attack, fast_attack, manual
    if (ret_output < 0) { 
    ...
    ret_output = iio_channel_attr_write(chnn_altvoltage0_output, "frequency", "3500000000");//# Receive path AGC Options: slow_attack, fast_attack, manual
    if (ret_output < 0) { 
       perror("Error setting frecuency rate RX: [Transmisor]"); 
        return ret_output;
    }
    ...
    printf("1-->%d \n", (int16_t)txSignal[0].re);
    printf("2-->%d \n", ((int16_t)txSignal[0].re) << 4);
    printf("2-->%d \n", 4095 << 4);
...
void complexExp(complex_t *signal, int N, double Fc, double Fs) {
    ...
    for (int n = 0; n < N; n++) {
        if(count<1){
            signal[n].re = 0x0fff ;//1* (pow(2, 12)-1);
            signal[n].im = 0x0fff;//1* (pow(2, 12)-1);
        }
        else if(count>1 && count<pri){
            signal[n].re = 0x0000;
            signal[n].im = 0x0000;
        }
        count++;

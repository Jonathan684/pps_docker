...
int num_lines(FILE *archivo);
int read_config_tx(char *config[], char *values[],int num_linea,FILE *archivo);
...
int main(){
    ...
    txbuf = iio_device_create_buffer(dev_tx, TxBufferSize, false);//Paso :0 Fin :-1225617408  Paso :-1225617408 Fin :-1225617408
    if (!txbuf) {
...
int configuracion_tx(){
    ...
    FILE *archivo;
    char set_file[50] ="transmition_config.txt";
    int num_linea;
    // Abre el archivo en modo de lectura ("r")
    archivo = fopen(set_file, "r");
   ...
   //printf("1 config[3], values[3] %s %s \n",config[3], values[3]);
...
int configuracion_rx(){
    ...
    FILE *archivo;
    char set_file[50] ="receive_config.txt";
    int num_linea;
    // Abre el archivo en modo de lectura ("r")
    archivo = fopen(set_file, "r");
    // Comprueba si el archivo se abrió correctamente
    ...
    //printf("configurando rx \n");
    num_linea = num_lines(archivo); /* numero de lineas de archivo */
...
int read_config_tx(char *config[], char *values[],int num_linea,FILE *archivo) {
    ...
    //int num_linea = num_lines(archivo);
    //printf("num_linea : %d\n",num_linea);

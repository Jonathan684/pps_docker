...
int main (int argc, char **argv)
{
	...
	// BUFFER de recepcion               num de muestras
	rxbuf = iio_device_create_buffer(rx, 1024*1024, false);
	if (!rxbuf) {
		perror("Could not create RX buffer");
		shutdown();
	}
	// BUFFER de transmicion             num de muestras
	...
	while (!stop)
	{
		...
		// Programar búfer de TX
		nbytes_tx = iio_buffer_push(txbuf);
		if (nbytes_tx < 0) { printf("Error pushing buf %d\n", (int) nbytes_tx); shutdown(); }

		// Rellenar el búfer de RX
		nbytes_rx = iio_buffer_refill(rxbuf);
		if (nbytes_rx < 0) { printf("Error refilling buf %d\n",(int) nbytes_rx); shutdown(); }

		//LEER: Obtenga punteros a RX buf y lea IQ desde el puerto 0 de RX buf
		p_inc = iio_buffer_step(rxbuf);
		p_end = iio_buffer_end(rxbuf);
		for (p_dat = (char *)iio_buffer_first(rxbuf, rx0_i); p_dat < p_end; p_dat += p_inc) {
			// Ejemplo: intercambiar I y Q
		...
		// ESCRIBIR: obtenga punteros a TX buf y escriba IQ en el puerto 0 de TX buf
		p_inc = iio_buffer_step(txbuf);
		p_end = iio_buffer_end(txbuf);
		for (p_dat = (char *)iio_buffer_first(txbuf, tx0_i); p_dat < p_end; p_dat += p_inc) {
			// Ejemplo: llenar con ceros
			//La muestra de 12 bits debe estar alineada con MSB, así que cambie por 4
			// https://wiki.analog.com/resources/eval/user-guides/ad-fmcomms2-ebz/software/basic_iq_datafiles#binary_format
			((int16_t*)p_dat)[0] = 0 << 4; // Real (I)
			((int16_t*)p_dat)[1] = 0 << 4; // Imag (Q)
		}

		//char *aux = iio_buffer_get_data(rxbuf);
		//printf(" %s\n",iio_buffer_get_data(rxbuf););
		// int16_t *data = iio_buffer_get_data(rxbuf);
		// int i;
    	// for (i = 0; i < (1024*1024); i++) {
       	//  printf("Sample %d: %d\n", i, data[i]);
    	// }
		// Incremento de contador de muestra y salida de estado
